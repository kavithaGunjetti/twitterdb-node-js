...
app.get('/user/tweets/feed', authenticateToken, async (request, response) => {
  ...
  const {user_id, name, usename, gender} = payload
  console.log(name)
...
app.get('/tweets/:tweetId', authenticateToken, async (request, response) => {
  ...
  const {payload} = request
  const {use_id, name, username, gender} = payload
  ...
  if (
    ...
    const getTweetDetailsQuery = `
        ...
        WHERE 
           tweet.tweet_id = ${tweetId} AND tweet.user_id = ${
             (userFollowers[0], user_id)
           }`
...
app.get(
  ...
  async (request, response) => {
    ...
    const getLikedUsersQuery = `
        ...
        *
        FROM
         follower INNER JOIN tweet ON tweet.user_id = follower.user_id INNER JOIN like ON
...
app.delete('/tweets/:tweetId', authenticateToken, async (request, response) => {
  ...
  const {payload} = request
  const {user_id, name, username, gender} = payload
  const selectUserQuery = `SELECT * FROM tweet WHERE tweet.user_id =${user_id}
